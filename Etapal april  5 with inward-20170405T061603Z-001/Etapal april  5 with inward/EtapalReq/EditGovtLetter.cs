...
namespace EtapalReq
{
    public partial class EditGovtLetter : Form
    {

        SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["DB"].ToString());
        SqlCommand cmd;

        public EditGovtLetter()
        {
            InitializeComponent();
        }


        private void EditGovtLetter_Load(object sender, EventArgs e)
        {
            try
            {
                //Today Date
        ...
        //        string UpdateData = "Update tblGovernmentLetter_IN"
        //                            + " set Letter_No = N'" + txtLetterNumber.Text + "',"
        ...
        private void btnSearchGLNo_Click(object sender, EventArgs e)
        {
            try
            {
                if (txtSearchGl.Text == "")
        ...
        private void Search_Govt_2017()
        {
            try
            {
                txtDeptOthers.Visible = true;
                string s = "";
                string Common1 = "Select Inward_No,Letter_No,Applicant_Name,Received_Date,Letter_Date,Entry_Date,Subject,FromDept,FromOffice, "
                                    + " ToSection,ToSubsection,ToCaseWorker,From_Other_Dept from ";
                string Common2 = "";
                ...
                cmd.Parameters.AddWithValue("@GL_No_Id", txtSearchGl.Text);
                conn.Open();
                ...
                if (dr.Read())
                {
                    ...
                    txtApplicantName.Text = dr[2].ToString();
                    dateTimePickerReceivedDate.Text = dr[3].ToString();
                    dateTimePickerLetterDate.Text = dr[4].ToString();
                    dateTimePickerEntryDate.Text = dr[5].ToString();
                    txtSubject.Text = dr[6].ToString();
                    cbDept.Text = dr[7].ToString();
                    cbOffice.Text = dr[8].ToString();
                    cbToSection.Text = dr[9].ToString();
                    cbToSubsection.Text = dr[10].ToString();
                    cbToCaseWorker.Text = dr[11].ToString();
                    txtDeptOthers.Text = dr[12].ToString();

                    grpGovernment.Visible = true;
                    lblGlNo.Visible = true;
                    lblGlNo.Text = "The Entered GL Number is " + txtSearchGl.Text + "";

                }
                else
                {
                    MessageBox.Show("No Document's Found for your entered Gl Number : " + "" + txtSearchGl.Text + "" + Environment.NewLine + "Enter Valid GL Number");
                    grpGovernment.Visible = false;
        ...
        private void Save_Govt_2017()
        {
            try
            {
                ...
                string Common2 = "";
                // string Common2 = "   tblGovernmentLetter_IN ";
                string Common3 = " set Letter_No = N'" + txtLetterNumber.Text + "',"
                                     + " Received_Date = @Received_Date,"
                                     + " Letter_Date = @Letter_Date,"
                                     + " FromDept = N'" + cbDept.Text + "',"
                                     + " From_Other_Dept = N'" + txtDeptOthers.Text + "',"
                ...
                //string Common4 = "  set Court_Case_Letter = @Court_Case_Letter,"
                ...
                if (i > 0)
                {
                    MessageBox.Show("Details Update Successfully for GL No : " + txtSearchGl.Text);
                }
                conn.Close();
                grpGovernment.Visible = false;
            }
        ...
        private void btnGotoGovtLetterPagee_Click(object sender, EventArgs e)
        {
            try
            {
                var Home = new EditCourtCaseLetter();
                Home.Show();
                this.Hide();
            }
            catch (Exception)
