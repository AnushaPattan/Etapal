...
namespace EtapalReq.Images
{
    public partial class GeneralLetter_WO : Form
    {
      ...
      // SqlConnection conn = new SqlConnection(@"data source=VMD-PC\SQLEXPRESS; initial catalog=EtapalReqWO; integrated Security=true;");
       // SqlConnection conn = new SqlConnection(@"data source=SOUTH-PC;initial catalog=EtapalReqWO;integrated security=true");
        ...
        public GeneralLetter_WO()
        {
            InitializeComponent();
        }

        private void GeneralLetter_WO_Load(object sender, EventArgs e)
        {
            try
            {
               ...
               // radDOLetter.Checked = true;
                ...
                int m = 0;
                string Number1 = "Select count(GL_No_Id) from tblGeneralLetter_WO";
        ...
        private void Save_General_Letter()
        {
            try
            {
                ...
                string InsertData = "Insert into tblGeneralLetter_WO(General_Letter,Applicant_Name,Hobli,Village,Received_Date,Letter_Date,Letter_No,Survey_No,From_Other_Dept,ToSection,ToCaseWorker,Subject)" +
                               "values(@General_Letter,N'" + txtApplicantName.Text + "',N'" + txtHobli.Text + "',N'" + txtVillage.Text + "',@Received_Date,@Letter_Date,@Letter_No,N'" + txtSurveyNo.Text + "',N'" + txtDeptOthers.Text + "',N'" + cbToSection.Text + "',N'" + cbToCaseWorker.Text + "',N'" + txtSubject.Text + "')";
        ...
        private void txtSubject_Click(object sender, EventArgs e)
        {
            try
            {
                txtSubject.Text = " ಅರ್ಜಿದಾರರ ಹೆಸರು : " + txtApplicantName.Text + Environment.NewLine + " ಹೋಬಳಿ : " + txtHobli.Text + Environment.NewLine + "ಗ್ರಾಮ : " + txtVillage.Text + Environment.NewLine + Environment.NewLine + "ಪತ್ರದ ಸಂಖ್ಯೆ : " + txtLetterNumber.Text + Environment.NewLine + " ಸಮೀಕ್ಷೆ ಸಂಖ್ಯೆ : " + txtSurveyNo.Text;
            }
            catch (Exception)
            {
